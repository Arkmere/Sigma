...
- `src/app/ui/anatomy-navigator.ts`: supplementary whole-body visual controller with family-specific presentation geometry mapped only to existing first-level semantic anatomy paths.
...
Semantic anatomy discovery is an optional in-form presentation layer. It derives child regions and exact-path facts from the registry rather than maintaining UI fact lists. Region panels switch in place, so navigation does not recreate the form or discard entered values; fact selection updates the existing canonical select and invokes its authoritative default-unit and guidance update. The browser exists only inside the existing manage-authorized creation surface. Its open/current-path state is transient, and presentation-family selection is not an input to discovery. Interactive SVG anatomy remains the separate 6F2 increment.
