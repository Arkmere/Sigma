...
- Consent is a same-device simulation, not authenticated remote consent; there is no cloud sync, real accounts/payments, external integration or production child-law policy. Cross-device sharing (Ticket 10) is a manually exchanged, passphrase-encrypted export file, not a live sync or account relationship — a lost passphrase makes that export permanently unrecoverable, and there is no key-exchange protocol.
