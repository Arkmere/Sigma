...
export function mountApp(root: HTMLElement, service = new SigmaService(new LocalStorageRepository(globalThis.localStorage))): void {
  ...
  const render = () => {
    ...
    const onFormAsync=(selector:string,action:(form:HTMLFormElement)=>Promise<void>,message?:string)=>{root.querySelector<HTMLFormElement>(selector)?.addEventListener('submit',(event)=>{event.preventDefault();const form=event.currentTarget as HTMLFormElement;action(form).then(()=>{if(message)notice={kind:'success',message};render();}).catch((error:unknown)=>{if(error instanceof Error){notice={kind:'error',message:error.message};render();return;}throw error;});});};
    ...
    onFormAsync('#fitcard-import-form',async(form)=>{const data=new FormData(form);const file=data.get('file');if(!(file instanceof File)||!file.size)throw new Error('Choose a fit card file.');await importFitCardFile(service,file,field(data,'passphrase'));},'Fit card imported.');
