...
export function mountApp(root: HTMLElement, service = new SigmaService(new LocalStorageRepository(globalThis.localStorage))): void {
  ...
  const render = () => {
    ...
    const showAnatomyPath=(path:string)=>{root.querySelectorAll<HTMLElement>('[data-anatomy-panel]').forEach((panel)=>panel.hidden=panel.dataset.anatomyPanel!==path);};
    ...
    bind(root,'[data-anatomy-path]','click',(element)=>{showAnatomyPath(element.dataset.anatomyPath!);root.querySelector<HTMLElement>(`[data-anatomy-panel="${element.dataset.anatomyPath}"] .anatomy-back`)?.focus();});
