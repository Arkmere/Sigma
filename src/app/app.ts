...
export function mountApp(root: HTMLElement, service = new SigmaService(new LocalStorageRepository(globalThis.localStorage))): void {
  ...
  const render = () => {
    ...
    const showAnatomyPath=(path:string)=>{root.querySelectorAll<HTMLElement>('[data-anatomy-panel]').forEach((panel)=>panel.hidden=panel.dataset.anatomyPanel!==path);const broad=path==='Body'?'':path.split(' > ').slice(0,2).join(' > ');root.querySelectorAll<SVGElement>('[data-anatomy-visual-path]').forEach(region=>region.toggleAttribute('data-selected',region.dataset.anatomyVisualPath===broad));root.querySelectorAll<HTMLElement>('.anatomy-region-choice').forEach(region=>region.toggleAttribute('aria-current',region.dataset.anatomyPath===broad));};
    ...
    bind(root,'[data-anatomy-visual-path]','click',(element)=>{element.dataset.anatomyPath=element.dataset.anatomyVisualPath;navigateAnatomy(element);});
