...
export class SigmaService {
  ...
  selectProfile(profileId: string): void { this.ensureWritable(); this.requireProfile(profileId); if(this.profileAccess(profileId)==='hidden') throw new Error('You cannot view that profile.'); this.data.activeProfileId = profileId; this.persist(); }
  ...
  grantableRecords(ownerProfileId:string) {
    ...
    if(!canManageProfile(this.data,actor.id,ownerProfileId))throw new Error('You cannot grant access for that profile.');
    return {
