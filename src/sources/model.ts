...
export interface ImportCandidate { sourceId:SourceId; externalFieldId:string; targetKind:'measurement'|'standard_size'; measurementType?:string; category:string; label:string; value:number|string; unitOrSystem:string; measuredAt:string; sourceItemId?:string; sourceDevice?:string; confidence?:number; derivation:Derivation; }
export type ImportRejectionReason = 'not_allowlisted'|'invalid_value'|'unsupported_unit'|'missing_required_field'|'invalid_provenance';
