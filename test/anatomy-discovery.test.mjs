...
test('read-only profiles expose no anatomy discovery or record form and navigator state is not persisted',()=>{
  ...
  const backup=service.exportBackup();assert.equal(backup.schemaVersion,6);assert.doesNotMatch(JSON.stringify(backup),/discoveryMode|currentAnatomyPath|anatomy-discovery/);
});
