...
test('unrelated facts retain the prototype and illustration data never enters persistence',()=>{
 const deferred=['weight','wrist-circumference','hand-length','hand-width','palm-circumference','finger-circumference','foot-length','foot-width','foot-circumference'];for(const id of deferred)for(const family of anatomyModelFamilyIds){const asset=anatomyAssetFor(fact(id),family);assert.equal(asset.familyId,'neutral');assert.equal(asset.assetRef,'/anatomy-model.svg');assert.ok(asset.symbolId);assert.match(renderAnatomyIllustration(`measurement.${id}`,family),/<use class="anatomy-model" href="\/anatomy-model\.svg#/);}
