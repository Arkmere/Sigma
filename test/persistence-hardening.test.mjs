...
const data=(schemaVersion=6)=>({schemaVersion,activeProfileId:'p',activeActorProfileId:'p',profiles:[profile()],measurements:[measurement()],standardSizes:[size()],brandFits:[],families:[],familyMemberships:[],adultConnections:[],sharingGrants:[],importedFitCards:[]});
...
test('schema-3 canonical mapping requires permitted units and no same-profile collision',()=>{
  ...
  assert.deepEqual(raw,before); assert.equal(result.data.schemaVersion,6);
});
...
test('backup export refuses unsafe repository states and preserves raw storage',()=>{
  for(const raw of [{schemaVersion:6,bad:true},{schemaVersion:99,future:true}]){
  ...
  const local=memory();const service=new SigmaService(new LocalStorageRepository(local));assert.equal(service.exportBackup().schemaVersion,6);
});
