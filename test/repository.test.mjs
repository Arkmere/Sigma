...
test('empty storage is distinguished and valid schema-1 data migrates to schema 4', () => {
  ...
  assert.equal(result.status, 'ok'); assert.equal(result.data.schemaVersion, 6); assert.equal(result.data.profiles[0].displayName, 'Alex'); assert.equal(result.data.activeActorProfileId,'p1'); assert.deepEqual(result.data.sharingGrants,[]);
});
